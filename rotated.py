...
validation_images_rotated = []
validation_labels = []
for im in glob.glob("train+validation\*.png"):
    im_name = im.lstrip(r"train+validation\ ")
    if im_name in validation_images_names:
        im = cv2.imread(im, cv2.ROTATE_90_CLOCKWISE)
        validation_images_rotated.append(im.flatten())
...
model.fit(train_images_rotated, train_labels)
validation_predictions = model.predict(validation_images_rotated)
print("NB:", end = " ")
